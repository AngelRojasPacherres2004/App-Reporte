...
class PerfilActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        currentRole = intent.getStringExtra(NavigationUtils.USER_ROL)?.lowercase() ?: "usuario"
        ...
        bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.nav_gestion -> {
                    ...
                    gestionIntent.putExtra(NavigationUtils.USER_EMAIL, tvEmail.text.toString())
                    gestionIntent.putExtra(NavigationUtils.SCHOOL_ID, tvSchool.text.toString())
                    startActivity(gestionIntent)
                    finish()
                    true
                }
                R.id.nav_foro -> {
                    NavigationUtils.navigateToForo(
                        this,
                        tvEmail.text.toString(),
                        currentRole,
                        tvSchool.text.toString()
                ...
                R.id.nav_asistente -> {
                    NavigationUtils.navigateToAsistente(
                        this,
                        tvEmail.text.toString(),
                        null // studentId not available here directly
                    )
                    finish()
                    true
                }
    ...
    private fun loadProfileData() {
        val intentEmail = intent.getStringExtra(NavigationUtils.USER_EMAIL) ?: ""
